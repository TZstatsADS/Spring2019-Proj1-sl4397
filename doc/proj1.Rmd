...
Then, I will divide people into two groups: single and married, and explore whether these two groups of people perceive happiness through different events. First, comparing wordclous of two groups of people can roughly give us a sense of what defines their happiness.
...
```{r single wordcloud, warning=FALSE, message=FALSE, echo=FALSE}
set.seed(0)
...
By looking at the wordcloud of the single group, we can see that, similar to the overall wordcloud, friend, family and home are the most important words. Also, we can see words like game, dinner, birthday, movie and shopping in the surrounding. That means leisure and enjoy-the-moment types also largely contribute to the happiness moments of single people.

```{r married wordcloud, warning=FALSE, message=FALSE, echo=FALSE}
set.seed(0)
...
From the wordcloud of married people, friend is also a significant word. However, instead of leisure-related words, we can see more words that are family related, including husband, wife, son, daughter, kids and baby. Hence, we find that most happy moments of married people are related to family and friends.
...
married_plot <- m.data %>%
  ...
  geom_col(fill = "coral")+
  geom_text(aes(x=predicted_category, label = round(n/sum(n),2), vjust=-0.3)) +
  labs(title = "Married", y = "frequency")+
  scale_y_continuous(limits=c(0,0.45), breaks = seq(0,0.5,0.1))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))


grid.arrange(single_plot, married_plot, nrow = 1)
```

From the barplot, we can see that single people achieve happiness more through achievements than affection while the married group, on the countrary, defines happiness more through affection. When comparing two barplots, we can see that the distribution of married group is approaximately 8% lower in achievement, 16% higher in affection, and slightly lower or equal in the other categories than the single group. That means after marriage, definition of happiness tend to switch to the categories of affection and mostly switch from the category of achievement.
